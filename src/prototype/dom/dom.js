import { extend, isElement, toHTML, isUndefined, isString, isNumber, isFunction, isArray } from '../lang/object';
...
extend(GLOBAL.Element, oldElement);
...
function isContent(content) {
  if (isUndefined(content) || content === null) return false;

  if (isString(content) || Object.isNumber(content)) return true;
  if (isElement(content)) return true;
...
extend(ATTRIBUTE_TRANSLATIONS.read.values, {
  href:        _getAttr2,
  src:         _getAttr2,
  type:        _getAttr,
  action:      _getAttrNode,
...
function addMethodsToTagName(tagName, methods) {
  ...
  extend(ByTag[tagName], methods);
}

function mergeMethods(destination, methods, onlyIfAbsent) {
  if (isUndefined(onlyIfAbsent)) onlyIfAbsent = false;
...
function addMethods(methods) {
  ...
  if (!tagName) {
    extend(Element.Methods, methods || {});
  } else {
    if (isArray(tagName)) {
  ...
  extend(Element, Element.Methods.Simulated);
